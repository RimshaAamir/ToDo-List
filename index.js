...
function creatNewTask(txt, ischeck, myId){
        ...
        deleteTask.addEventListener('click', function () {    // by using filter
            itemsData = itemsData.filter(item => item.id !== myNewTask.id);
            localStorage.setItem('myAllItemsData', JSON.stringify(itemsData));
            myNewTask.remove();
        });
        ...
        //     myNewTask.remove();
            ...
            //     item.id === myNewTask.id  ? { ...items, text: "Updated Task 6" } : task
            // );
        // });
...
function toggleTaskContainer(checkbox, task, description) {
    ...
    itemsData = itemsData.map(item => 
        item.id == task.id ? { ...item, isC: checkbox.checked } : item
    );
    localStorage.setItem('myAllItemsData', JSON.stringify(itemsData));
...
function ok(myThis) {  
    ...
    itemsData = itemsData.map(item =>
        item.id == myCurrentTask.id ? { ...item, txtP: paragraph.textContent } : item
    );
    localStorage.setItem('myAllItemsData', JSON.stringify(itemsData));
...
function loadData() {
    ...
    itemsData = mydata.map((item, index) => {
        let newItem = creatNewTask(item.txtP, item.isC, index + 1);
        if (item.isC) {
            document.getElementById('checked').appendChild(newItem);
            newItem.querySelector('p').style.textDecoration = 'line-through';
        } else {
            document.getElementById('unchecked').appendChild(newItem);
        }
        return {
            ...item,
            id: index + 1
